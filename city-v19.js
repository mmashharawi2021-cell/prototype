...
function rng(seed=1){let a=seed>>>0;return()=>{a|=0;a=(a+0x6D2B79F5)|0;let t=Math.imul(a^(a>>>15),1|a);t=(t+Math.imul(t^(t>>>7),61|t))^t;return((t^(t>>>14))>>>0)/4294967296}}const R=rng(19092027);
...
const route=new THREE.CatmullRomCurve3([
 new THREE.Vector3(30,0,78),new THREE.Vector3(22,0,30),new THREE.Vector3(-34,0,-68),new THREE.Vector3(-12,0,-148),new THREE.Vector3(38,0,-244),new THREE.Vector3(14,0,-330),new THREE.Vector3(-32,0,-425),new THREE.Vector3(0,0,-555)
],false,'catmullrom',.45);
...
function pushJag(base,x,y,z,sx,sy,sz,rx=0,ry=0,rz=0){P.set(x,y,z);Q.setFromEuler(new THREE.Euler(rx,ry,rz));S.set(sx,sy,sz);const lm=new THREE.Matrix4().compose(P,Q,S);jagged.push(base.clone().multiply(lm))}
function buildingAt(q,side,w,d,floors,damage,hero=false){const f=frameAt(q),dist=10.8+w*.52+R()*3.2,c=f.p.clone().addScaledVector(f.n,side*dist);const ang=Math.atan2(f.t.x,f.t.z)+(side<0?Math.PI:0),base=new THREE.Matrix4().makeRotationY(ang);base.setPosition(c.x,0,c.z);const fh=3.75;
 push(3,base,0,floors*fh*.37,0,w*.42,floors*fh*.66,d*.44);
 ...
 for(let fl=0;fl<floors;fl++){
   ...
   for(let bx=0;bx<baysX;bx++){
     ...
     if(keep){
       ...
       if(R()>.34)pushJag(base,cx+(R()-.5)*bw*.38,y+fh*(.30+R()*.52),-d/2-.18,bw*(.16+R()*.30),fh*(.12+R()*.26),.22,(R()-.5)*.6,(R()-.5)*.22,(R()-.5)*.7);
     }
 ...
 for(let i=0;i<(hero?6:2);i++) if(R()<.82){push(R()>.5?0:1,base,(R()-.5)*w*.58,floors*fh*(.62+R()*.38),(R()-.5)*d*.60,w*(.14+R()*.30),.16,d*(.12+R()*.30),(R()-.5)*.34,(R()-.5)*.18,(R()-.5)*.40);if(hero&&R()>.35)pushJag(base,(R()-.5)*w*.62,floors*fh*(.68+R()*.30),(R()-.5)*d*.62,w*(.10+R()*.22),fh*(.10+R()*.18),d*(.10+R()*.22),(R()-.5)*.8,(R()-.5)*.4,(R()-.5)*.8)}
 for(let i=0;i<(hero?8:4);i++){push(R()>.5?2:0,base,(R()-.5)*w*.75,.35+R()*1.4,(R()-.5)*d*.80,w*(.10+R()*.28),.12,d*(.10+R()*.30),(R()-.5)*.7,(R()-.5)*.5,(R()-.5)*.8)}
}

const heroQs=[.03,.10,.18,.27,.36,.46,.56,.66,.76,.86,.94,.985];for(let i=0;i<heroQs.length;i++){const q=heroQs[i];for(const side of[-1,1]){const w=18+R()*10,d=15+R()*8,f=7+Math.floor(R()*8),damage=.50+R()*.22;buildingAt(q+(R()-.5)*.014,side,w,d,f,damage,true)}}
...
const jagGeom=new THREE.TetrahedronGeometry(1,0),jagMesh=new THREE.InstancedMesh(jagGeom,mats[1],jagged.length);jagged.forEach((m,i)=>jagMesh.setMatrixAt(i,m));jagMesh.instanceMatrix.needsUpdate=true;jagMesh.frustumCulled=false;scene.add(jagMesh);
...
function draw(now){requestAnimationFrame(draw);const dt=Math.min((now-last)/1000,.05);last=now;const max=document.documentElement.scrollHeight-innerHeight,pct=max>0?scrollY/max:0,t=pct*10.005;
 const cp=clamp((t-4.82)/(10.005-4.82)),q=Math.min(.975,ease(cp*(.86+.11*cp))),f=frameAt(q),ahead=frameAt(Math.min(.995,q+.055));const desired=f.p.clone();desired.y=lerp(33,14,cp);desired.addScaledVector(f.n,Math.sin(cp*Math.PI*4)*.22);const target=ahead.p.clone();target.y=lerp(11,7,cp);
 if(!camInit){camPos.copy(desired);look.copy(target);camInit=true}else{camPos.lerp(desired,1-Math.exp(-18*dt));look.lerp(target,1-Math.exp(-16*dt))}camera.position.copy(camPos);camera.lookAt(look);camera.fov=lerp(58.5,65.5,ease(cp));camera.updateProjectionMatrix();scene.fog.near=lerp(38,20,cp);scene.fog.far=lerp(390,255,cp);scene.fog.color.set(0xa9afaa).lerp(new THREE.Color(0x9ba39e),cp*.30);renderer.toneMappingExposure=lerp(.99,.93,cp);sky.position.x=camera.position.x*.15;renderer.render(scene,camera);if(!ready){ready=true;window.dispatchEvent(new Event('atlas-city-ready'))}}
